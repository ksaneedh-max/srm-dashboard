...
const STORAGE_FILE =
  process.env.STORAGE_STATE_PATH || path.resolve(__dirname, "state.json");
...
async function ensureBrowser() {
  try {
    ...
    if (!context) {
      console.log("Context missing → recreating");
      await createContextAndPage();
      return;
    }
...
async function handleSessionLimit(page) {
  try {
    ...
    if (await terminateBtn.count()) {
      console.log("Session limit reached → terminating old sessions");
...
async function loadAttendance(page) {
  console.log("Opening attendance section...");
...
app.get("/marks", async (req, res) => {
  ...
  try {
    ...
    if (!loaded) {
      return res.json({ error: "Marks page failed to load" });
    }
    ...
    if (!marksFrame) {
      return res.json({ error: "Marks frame not found" });
    }
